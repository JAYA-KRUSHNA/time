...
export default function AdminForgotPasswordPage() {
    ...
    const handleFindAccount = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/auth/forgot-password', {
                ...
                body: JSON.stringify({ email, action: 'find' }),
            });
            const data = await res.json();
            if (!res.ok) {
                setErrors({ email: data.error || 'Account not found' });
                return;
            }
    ...
    const handleVerifyOTP = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/auth/verify-otp', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: foundEmail, code: otp, purpose: 'password_reset' }),
            });
            const data = await res.json();
            if (!res.ok) {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/auth/forgot-password', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: foundEmail, action: 'reset', password: newPassword }),
            });
            if (!res.ok) {
                const data = await res.json();
    ...
    return (
        <AuthLayout title="Reset Password" subtitle={
            step === 'email' ? 'Enter your admin email to find your account' :
            ...
            {step === 'email' && (
                <form onSubmit={handleFindAccount}>
                    ...
                    <div style={{ textAlign: 'center', marginTop: 20 }}>
                        <Link href="/auth/admin/login" className="link-accent" style={{ fontSize: 13 }}>
                            Back to Login
                        </Link>
                    </div>
                </form>
